...
class PlayerDelegate2(
    ...
    // 未登陆：48[480P 清晰]及以下
    // EN: Not logged in: 48 [480P] and below
    // 已登陆无大会员：80[1080P 高清]及以下
    // EN: Logged in without VIP: 80 [1080P] and below
    // 大会员：无限制
    // EN: VIP member: no limit
    ...
    // EN: Video format: 0:flv,1:mp4,4048:dash
    ...
    // EN: Last recorded playback position
    ...
    override fun onCreate() {
        ...
        //音频焦点冲突时是否释放
        // EN: Whether to release on audio focus conflict
    ...
    override fun getMediaSource(
        ...
        return when (dataSourceArr[0]) {
            "[local-merging]" -> {
                // 本地音视频分离
                // EN: Local audio/video separation
...
//        }
    ...
    suspend fun loadPlayerSource(
        ...
        try {
            ...
            if (lastPosition > 0L) {
                ...
                // EN: Loop videos don't resume playback
                ...
                // 从进度0开始记录播放历史
                // EN: Start recording playback history from position 0
                historyReport(0L)
            }
    ...
    override fun openPlayer(source: BasePlayerSource) {
        ...
        playerCoroutineScope.launch(Dispatchers.Main) {
            ...
            views.videoPlayer.setSpeed(speed, true)
            // 播放倍速提示
            // EN: Playback speed tip
            if (speed != 1f) {
                GlobalToaster.show("注意，当前为${speed}倍速")
            }
        }
        // 播放器是否默认全屏播放
        // EN: Whether player defaults to fullscreen
        controller.checkIsPlayerDefaultFull()
        // 是否显示分P和剧集按钮
        // EN: Whether to show P and episode buttons
        ...
        // 添加到用户库历史记录
        // EN: Add to user library history
    ...
    fun onPictureInPictureModeChanged(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (isInPictureInPictureMode) { // 进入画中画模式，则隐藏其它控件
                // EN: Entering PiP mode, hide other controls
                // 隐藏视频控制器
                // EN: Hide video controller
                views.videoPlayer.hideController()
                //
                views.videoPlayer.isPicInPicMode = true
                // 视频组件全屏
                // EN: Video component fullscreen
                scaffoldApp.fullScreenPlayer = true
                // 调整弹幕样式，调小字体，限制行数
                // EN: Adjust danmaku style, smaller font, limit lines
            } else {
