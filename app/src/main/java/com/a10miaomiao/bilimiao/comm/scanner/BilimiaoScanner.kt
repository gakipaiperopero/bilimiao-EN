...
object BilimiaoScanner {
    ...
    fun openScanner(
        ...
        // 优先启动bilimiao扫码器
        // EN: Prefer to start bilimiao scanner
        kotlin.runCatching {
            val intent = Intent()
            intent.setAction("bilimiao.plugin.ACTION_SCAN_QRCODE") // 与插件Manifest中定义的Action一致
            // EN: Consistent with the Action defined in the plugin Manifest
            ...
            if (it.isSuccess) {
                // 启动bilimiao扫码器成功
                // EN: Started bilimiao scanner successfully
                return true
            }
        }
        // 再尝试启动GMS扫码器
        // EN: Try to start GMS scanner
        kotlin.runCatching {
            val intent = Intent()
            ...
            if (it.isSuccess) {
                // 启动GMS扫码器成功
                // EN: Started GMS scanner successfully
                return true
            }
        }
        // 启动失败
        // EN: Start failed
        // 提示下载bilimiao扫码器
        // EN: Prompt to download bilimiao scanner
        resultCallback = null
        return false
    }
    ...
    fun onActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            if (data.hasExtra("extra_barcode_result")) {
                ...
                // bilimiao扫码器扫码结果
                // EN: bilimiao scanner scan result
