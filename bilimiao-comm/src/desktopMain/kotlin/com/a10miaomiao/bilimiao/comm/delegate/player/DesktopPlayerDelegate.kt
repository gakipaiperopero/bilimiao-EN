...
class DesktopPlayerDelegate(
    ...
    // EN: Player show/hide callback (set by Main.kt)
    var onShowPlayerChanged: ((Boolean) -> Unit)? = null

    // 播放参数
    // EN: Playback parameters
    ...
    // EN: Accumulated duration before current segment
    ...
    private fun loadAndPlay(source: BasePlayerSource) {
        ...
        // 先停止之前的播放
        // EN: Stop previous playback first
        progressJob?.cancel()
        player.stopPlayback()
        coroutineScope.launch {
            ...
            try {
                ...
                // EN: Build HTTP headers, pass to mediamp via UriMediaData
                ...
                when (resolved.format) {
                    PlaybackFormat.MERGING -> {
                        ...
                        val options = if (audioUrl != null) {
                            listOf(":input-slave=$audioUrl")
                        } else {
                            emptyList()
                        }
                        player.setMediaData(
                            UriMediaData(resolved.videoUrl, headers, options = options)
                        )
                    }
                    PlaybackFormat.SEGMENTED -> {
                        // 分段视频：播放第一段
                        // EN: Segmented video: play first segment
                    ...
                    PlaybackFormat.TEMP_MPD -> {
                        ...
                        // EN: [dash-mpd] format: write MPD XML to temp file for playback
                ...
                // EN: playUri only sets media data (state becomes READY), need to call resume to start playback
     ...
     * 将 MPD XML 内容写入临时文件
     */
    // EN: Write MPD XML content to temp file
    ...
    private fun startProgressTracking(source: BasePlayerSource) {
        ...
        progressJob = coroutineScope.launch {
            while (isActive) {
                ...
                // 每5秒上报历史记录
                // EN: Report history every 5 seconds
    ...
    fun setVolume(newVolume: Int) {
        ...
        _mediampPlayer?.features?.let { features ->
            (features[AudioLevelController])?.setVolume(clamped / 100f)
        }
    }
