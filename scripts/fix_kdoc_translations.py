#!/usr/bin/env python3
"""Fix multi-line KDoc blocks that have (see above) fallback translations."""
...
def extract_kdoc_lines(lines, start_idx, end_idx):
    """Extract individual Chinese text lines from a KDoc block."""
    texts = []
...
def translate_line(text):
    """Translate a single Chinese line using dictionary."""
    text = text.strip()
    if not text:
        return ""
...
for rel_path in FILES:
    ...
    while i < len(lines):
        ...
        if in_kdoc:
            if stripped.endswith("*/"):
                result.append(line)
                # Check if the next line is // EN: (see above)
                if i + 1 < len(lines) and "(see above)" in lines[i + 1]:
                    kdoc_texts = extract_kdoc_lines(lines, kdoc_start, i)
                    translations = []
                    for t in kdoc_texts:
                        en = translate_line(t)
                        if en:
                    ...
                    if translations:
                        en_comment = ". ".join(translations)
                        result.append(f"{indent}// EN: {en_comment}\n")
                    else:
    ...
    if file_fixed > 0:
        ...
        print(f"  {rel_path}: {file_fixed} kdoc translations fixed")

print(f"\nFixed {total_fixed} multi-line KDoc translations.")
