...
class VideoDetailViewModel(
    ...
    private fun autoStartPlay() = viewModelScope.launch(Dispatchers.Main) {
        ...
        if (basePlayerDelegate.getSourceIds().aid == arcData.aid.toString()) {
            // 同个视频不替换播放
            // EN: Don't replace if it's the same video
            return@launch
        }
        val openMode = SettingPreferences.mapPreferences {
            it[SettingPreferences.PlayerOpenMode] ?: SettingConstants.PLAYER_OPEN_MODE_DEFAULT
    ...
    fun playVideo(page: Page) {
        ...
        if (isAutoPlaySeason && ugcSeason != null) {
            ...
            if (playListFromId != ugcSeason.id.toString() ||
                ...
                // EN: When current playlist source is not the current collection or video is not in playlist, create new playlist
                // 以合集创建播放列表
                // EN: Create playlist from collection
            ...
            // EN: When current video is not in playlist: create new playlist if nothing is playing or list is empty, otherwise append to end
            if (playListStore.state.items.isEmpty()
                || playerStore.state.aid.isEmpty()) {
                // 以当前视频创建新的播放列表
                // EN: Create new playlist with current video
                ...
                // 将视频添加到播放列表末尾
                // EN: Add video to end of playlist
    ...
    fun addVideoHistoryToview() = viewModelScope.launch(Dispatchers.IO) {
        if (!userStore.isLogin()) {
            GlobalToaster.show("Please login first")
            return@launch
        }
        try {
            val arcData = detailData.value?.getArcData() ?: return@launch
            val res = BiliApiService.userApi
                ...
                .json<MessageInfo>()
            if (res.code == 0) {
                GlobalToaster.show("Added to Watch Later")
    ...
    fun requestLike(
        ...
        try {
            ...
            if (res.isSuccess) {
                ...
                if (state == 1) {
                    GlobalToaster.show("Liked")
                } else {
                    GlobalToaster.show("Like removed")
                }
    ...
    fun menuItemClick(item: MenuItemPropInfo) {
        ...
        when (item.key) {
            ...
            3 -> {
                ...
                GlobalToaster.show("Copied: $text")
            }
            4 -> {
                // 复制AV号
                // EN: Copy AV number
            ...
            11 -> {
                ...
                if (current != -1) {
                    ...
                    GlobalToaster.show("Added to Play Next")
                } else {
                    GlobalToaster.show("Failed to add, cannot find currently playing video")
                }
            }
            12 -> {
                // 添加至最后一个播放
                // EN: Add to end of playlist
                playListStore.run {
                    addItem(
                        videoArc.toPlayListItem(viewPages),
                        state.items.size,
                ...
                GlobalToaster.show("Added to end of playlist")
            }
            13 -> {
                // 添加至稍后再看
                // EN: Add to Watch Later
