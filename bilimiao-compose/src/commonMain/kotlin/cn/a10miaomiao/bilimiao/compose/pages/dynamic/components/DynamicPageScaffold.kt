...
private fun DynamicAllAndVideoWrap(
    ...
    val scope = rememberCoroutineScope()
    val tabs = listOf(
        PageTabIds.DynamicAll to "Dynamics",
        PageTabIds.DynamicVideo to "Videos",
    )
    ...
    Column(
        ...
        if (isMiniUpList) {
            TabRow(
                ...
                // 右侧“最常访问”按钮
                // EN: "Most visited" button on the right
                TextButton(
                    onClick = toUpper,
...
fun DynamicUpperWrap(
    ...
    Row(
        ...
        // 左侧为所选UP主的动态列表占位
        // EN: Left side is placeholder for the selected UP's dynamic list
        Box(
            modifier = Modifier
                .weight(1f)
                .fillMaxHeight(),
