...
class VideoDownloadDialogState(
    ...
    val description get() = qualityList.find { it.first == quality }?.second ?: "Not selected"
    ...
    fun startDownload() {
        if (quality == 0) {
            showSnackbar("Please select quality")
            return
        }
        val manager = downloadManager
        if (manager == null) {
            showSnackbar("Download service error")
            return
        }
        val videoArc = arcData
        if (videoArc == null) {
            showSnackbar("Missing video info")
            return
        }
        ...
        GlobalToaster.show("Successfully created ${checkedSize} download(s)")
        dismiss()
        _checkedMap.clear()
    }
                    ...
                    Row(
                        ...
                        Box() {
                            Button(
                                ...
                                Text(text = "Quality: " + state.description)
                            }
                            DropdownMenu(
                        ...
                        Button(
                            ...
                            Text(text = "Start download (${state.checkedSize})")
                        }
                    }
                }
            },
