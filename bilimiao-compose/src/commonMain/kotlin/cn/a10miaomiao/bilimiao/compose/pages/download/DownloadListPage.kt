...
internal class DownloadListPageViewModel(
    ...
    fun copyDownloadPathToClipboard() {
        ...
        GlobalToaster.show("Path copied to clipboard")
    }
}
...
internal fun DownloadListPageContent(
    ...
    if (showHelpDialog) {
        ...
        AlertDialog(
            ...
            title = { Text(text = "Download path") },
            text = {
                Column() {
                    Text(text = "Video download folder: ${downloadPath}")
                    Text(text = "Directory structure matches the official Bilibili app. Cache files can be shared between them.")
                    Text(text = "Use BiliDownOut to export video files")
                }
            },
            confirmButton = {
                Row() {
