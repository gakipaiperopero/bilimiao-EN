...
private class TelVerifyPageViewModel(
        ...
        try {
            ...
            if (res.isSuccess) {
                ...
                if (info.bind_tel && info.tel_verify) {
                    ...
                    messageDialog.alert(
                        text = "This account does not support phone or email verification. Please complete account info on the official Bilibili app or website and try again",
            ...
            GlobalToaster.show("Network error: $e")
            e.printStackTrace()
        }
    }
    ...
    fun verifyTel() = viewModelScope.launch(Dispatchers.Main) {
        try {
            if (verifyCode.value.isBlank()) {
                messageDialog.alert("Please enter verification code")
                return@launch
            }
            loading.value = true
            val res = withContext(Dispatchers.IO) {
    ...
    private suspend fun sendSms(
        ...
        if (res.isSuccess) {
            ...
            GlobalToaster.show("SMS verification code sent")
            return true
        } else {
            GlobalToaster.show(res.message)
            return false
    ...
    private suspend fun sendEmail(
        ...
        if (res.isSuccess) {
            startCountdown(60)
            captchaKey = res.requireData().captcha_key!!
            GlobalToaster.show("Email verification code sent")
            return true
        } else {
            GlobalToaster.show(res.message)
            return false
    ...
    /**
     * Captcha callback, send verification code
     */
    override suspend fun onResult(
        ...
        if (verifyType.value == VerifyType.TEL) {
            ...
            GlobalToaster.show("Unknown type")
            return false
        }
    }
...
private fun TelVerifyPageCompose(
    ...
    PageConfig(title = "Account verification")
    ...
    Box(
        ...
        Column(
            ...
            if (verifyType == TelVerifyPageViewModel.VerifyType.TEL) {
                Text(
                    text = "Phone: ${tmpAccountInfo?.hide_tel}",
                    fontSize = 14.sp,
                    textAlign = TextAlign.Center,
                    modifier = Modifier
                ...
                Text(
                    text = "Email: ${tmpAccountInfo?.hide_mail}",
                    fontSize = 14.sp,
                    textAlign = TextAlign.Center,
                    modifier = Modifier
                        .padding(bottom = 40.dp)
            ...
            Row(
                ...
                if (verifyType == TelVerifyPageViewModel.VerifyType.TEL
                    ...
                    TextButton(onClick = {
                        ...
                        Text(text = "Use email verification")
                    }
                }
                if (verifyType == TelVerifyPageViewModel.VerifyType.EMAIL
                    ...
                    TextButton(onClick = {
                        ...
                        Text(text = "Use phone verification")
                    }
                }
            }
