 ...
 */
// EN: Mark gesture event start
// EN: Coroutine scope for transformations
...
fun ZoomableViewState.onGestureEnd(scope: CoroutineScope, transformOnly: Boolean) {
    scope.apply {
        ...
        // 如果缩放比小于1，要自动回到1
        // EN: If scale ratio is less than 1, automatically return to 1
        // 如果缩放比大于最大显示缩放比，就设置回去，并且避免加速度
        // EN: If scale ratio exceeds max display scale, set it back and avoid velocity
        val nextScale = when {
            scale.value < 1 -> 1F
 ...
 * @return 是否消费这次事件
 */
// EN: Input gesture event
// EN: Coroutine scope for transformations
// EN: Gesture center coordinates
// EN: Gesture pan distance
// EN: Gesture zoom rate
// EN: Rotation angle
// EN: Event object
// EN: Whether to consume this event
fun ZoomableViewState.onGesture(
    ...
    // 这里只记录最大手指数
    // EN: Only record maximum finger count here
    if (eventChangeCount <= event.changes.size) {
        eventChangeCount = event.changes.size
    } else {
        // 如果手指数从多个变成一个，就结束本次手势操作
        // EN: If finger count goes from multiple to one, end this gesture operation
        return false
    }
    ...
    // EN: When using two fingers, if finger distance is below a threshold, zoom and rotation values become erratic, so skip them
    ...
    lastPan = pan
    // 记录手势的中点
    // EN: Record gesture centroid
    centroid = center
    // 计算边界，如果目标缩放值超过最大显示缩放值，边界就要用最大缩放值来计算，否则手势结束时会导致无法归位
    // EN: Calculate bounds; if target scale exceeds max display scale, use max scale for bounds, otherwise gesture end may fail to reset
    boundScale =
    ...
    // 如果手指数1，就是拖拽，拖拽受范围限制
    // EN: If finger count is 1, it is drag, drag is bound limited
    // 如果手指数大于1，即有缩放事件，则支持中心点放大
    // EN: If finger count > 1, there is zoom event, supports center-point zoom
    ...
    // 添加到手势加速度
    // EN: Add to gesture velocity tracker
    ...
    // EN: Check if reached boundary; if so, don't consume event, let parent handle it
    val canConsumeX = reachSide(pan.x, nextOffsetX, boundX)
    val canConsumeY = reachSide(pan.y, nextOffsetY, boundY)
    // 判断主要活动方向
    // EN: Determine main direction of movement
    ...
    // 返回true，继续下一次手势
    // EN: Return true, continue to next gesture
    return true
}
 ...
 * 追踪缩放过程中的中心点
 */
// EN: Track center point during zoom
