...
private class SearchByTypeContentViewModel(
    override val di: DI,
    val type: Int, // Users: 2, Live: 4, Articles: 6, Anime: 7, Movies & TV: 8,
    val keyword: String,
) : ViewModel(), DIAware {
    ...
    val userSortList = listOf(
        SearchByTypeRequest.UserSort.DEFAULT to "Default",
        SearchByTypeRequest.UserSort.FANS_DESCEND to "Fans high to low",
        SearchByTypeRequest.UserSort.FANS_ASCEND to "Fans low to high",
        SearchByTypeRequest.UserSort.LEVEL_DESCEND to "Level high to low",
        SearchByTypeRequest.UserSort.LEVEL_ASCEND to "Level low to high",
    )
    val userSort = mutableStateOf(userSortList[0])

    val userTypeList = listOf(
        SearchByTypeRequest.UserType.ALL to "All",
        SearchByTypeRequest.UserType.UP to "Creators",
        SearchByTypeRequest.UserType.NORMAL_USER to "Verified users",
        SearchByTypeRequest.UserType.AUTHENTICATED_USER to "Regular users",
    )
...
internal fun SearchByTypeContent(
    type: Int, // 用户：2，直播：4，图文：6，番剧：7，影视：8，
    // EN: Users: 2, Live: 4, Articles: 6, Anime: 7, Movies & TV: 8
    keyword: String,
    isActive: Boolean
) {
