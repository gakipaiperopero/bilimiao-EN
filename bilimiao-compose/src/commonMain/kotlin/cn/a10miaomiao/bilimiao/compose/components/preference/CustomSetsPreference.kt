                    ...
                    InputChip(
                        ...
                        onClick = {
                            if (valueCanEdit(text)) {
                                ...
                                GlobalToaster.show("Editing not allowed")
                            }
                        },
                        label = {
                            valueText(text)
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        ...
                        if (v == null || v <= 0) {
                            errorMessage.value = "Please enter a valid value"
                        } else if(v > 10) {
                            errorMessage.value = "Maximum 10x speed"
                        } else if(v in value.map { it.toDouble() }) {
                            errorMessage.value = "$v already exists"
                        } else {
                            onValueChange(setOf(
                                *value.toTypedArray(),
                                v.toString(),
                            ...
                            editDialogState.value = EditDialogState.Closed
                            GlobalToaster.show("Added successfully")
                        }
                    }
                ) {
                    Text(text = "Confirm add")
                }
            },
            dismissButton = {
                TextButton(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        ...
                        editDialogState.value = EditDialogState.Closed
                        GlobalToaster.show("Removed successfully")
                    }
                ) {
                    Text(
                        text = "Remove",
