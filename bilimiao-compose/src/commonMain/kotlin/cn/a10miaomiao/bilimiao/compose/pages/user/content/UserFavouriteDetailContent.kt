...
private class UserFavouriteDetailViewModel(
    ...
    private fun loadData(
        ...
        try {
            ...
            e.printStackTrace()
            list.fail.value = "Connection failed"
        } finally {
    ...
    fun openVideo(item: MediasInfo) {
        ...
        if (isAutoPlay.value) {
            if (ugcInfo != null) {
                ...
                GlobalToaster.show("Auto play only supports regular videos")
            }
        }
        if (ugcInfo != null) {
    ...
    fun addPlayList() {
        ...
        if (media == null) {
            GlobalToaster.show("Loading data, please try again later")
            return
        }
    ...
    fun unfavFolder() = viewModelScope.launch(Dispatchers.IO) {
        try {
            val res = BiliApiService.userApi
                ...
                .json<MessageInfo>()
            if (res.isSuccess) {
                GlobalToaster.show("Unsubscribed")
                refresh()
            } else {
                GlobalToaster.show(res.message)
            }
...
internal fun UserFavouriteDetailContent(
    ...
    val pageConfigId = PageConfig(
        title = if (keyword.isBlank()) "Favorite details" else "Search\n-\n${keyword}",
    ...
    if (showEditDialog && info != null) {
        ...
        AlertDialog(
            ...
            text = {
                Text(text = "Delete folder: ${mediaTitle}?")
            },
            confirmButton = {
                TextButton(
                    enabled = !loading,
