 ...
 */
// EN: AppBar state management
// EN: Used to control AppBar display and properties on the Compose side
class AppBarState {
    // 是否显示 AppBar
    // EN: Whether to show AppBar
    ...
    // 是否为导航菜单模式
    // EN: Whether in navigation menu mode
    var isNavigationMenu by mutableStateOf(false)
        internal set

    // 当前选中的菜单项key
    // EN: Currently selected menu item key
    ...
    // 底栏是否显示（用于竖屏模式）
    // EN: Whether bottom bar is visible (for portrait mode)
    var barVisible by mutableStateOf(true)
        internal set

    // 菜单是否展开（用于竖屏模式）
    // EN: Whether menu is expanded (for portrait mode)
 ...
 */
// EN: CompositionLocal for providing AppBarState in the component tree
val LocalAppBarState: ProvidableCompositionLocal<AppBarState?> = compositionLocalOf { null }

/**
 * AppBar DSL 构建器
 */
// EN: AppBar DSL builder
