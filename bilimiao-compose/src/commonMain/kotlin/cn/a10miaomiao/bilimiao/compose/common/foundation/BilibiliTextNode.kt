    ...
    return mapNotNull {
        ...
        when (node) {
            ...
            is TextNode.Text.Link -> {
                ...
                // EN: The first is usually a newline (0x0a), then the second is a form feed (0x0c)
                // 这里直接去除得了，然后是显示文字部分，到(0x11)结束
                // EN: Remove them directly, then comes the displayed text part, ending at (0x11)
                // 后面部分应该是控制格式样式的，现在不考虑，只显示文字
                // EN: The rest should be format control styles, not considered for now, only display text
                // ฅ^•ﻌ•^ฅ
                var showTextStart = 0
