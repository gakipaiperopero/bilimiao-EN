 ...
 * 弹幕渲染器实现
 */
// EN: Danmaku renderer implementation
class DanmakuRenderer(
    ...
    private inner class Consumer : IDanmakus.DefaultConsumer<BaseDanmaku>() {
        ...
        override fun accept(drawItem: BaseDanmaku): Int {
            ...
            // 跳过过滤弹幕（优先级>0的除外）
            // EN: Skip filtered (except priority > 0)
            ...
            // 跳过未到时间的弹幕，请求缓存构建
            // EN: Skip not-yet-due danmaku; request cache build
            if (drawItem.isLate()) {
            ...
            // 同屏弹幕密度只对滚动弹幕有效
            // EN: On-screen density only applies to scrolling danmaku
