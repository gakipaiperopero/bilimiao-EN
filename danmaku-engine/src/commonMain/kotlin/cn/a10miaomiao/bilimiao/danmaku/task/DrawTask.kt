 ...
 */
 // EN: Danmaku draw task. Core rendering loop implementation. Responsible for:. - Managing danmaku data list and screen display window. - Coordinating parser to load danmaku data. - Drawing visible danmaku through renderer. - Handling playback state changes, seeking, sync, etc.
open class DrawTask(
    timer: DanmakuTimer,
    ...
    /** 全局弹幕列表（按时间排序） */
    // EN: Global danmaku list (time-sorted)
    ...
    init {
        ...
        // 注册重复合并过滤器
        // EN: Register duplicate merge filter
     ...
     */
    // EN: Callback on removal (subclass can override)
    protected open fun onDanmakuRemoved(danmaku: BaseDanmaku) {
        // 子类可覆写
        // EN: Subclass can override
    }
     ...
     */
     // EN: Remove timed out live danmaku
    @Synchronized
    ...
    override fun getVisibleDanmakusOnTime(time: Long): IDanmakus {
        ...
        // EN: Avoid ConcurrentModificationException; retry up to 3 times
        var i = 0
        while (i++ < 3) {
            try {
     ...
     * 通过解析器加载弹幕数据
     */
     // EN: Load danmaku data via parser
     ...
     */
     // EN: Core rendering loop. 1. Clear retainer (if flagged). 2. Calculate current time window. 3. Get danmaku within screen window. 4. Draw running danmaku in sync mode first. 5. Draw danmaku within screen window. 6. Return rendering state
    protected open fun drawDanmakus(disp: IDisplayer, timer: DanmakuTimer): RenderingState? {
        ...
        // 隐藏状态且未请求渲染时直接返回
        // EN: Return if hidden and not requested
        ...
        // 计算可见时间窗口
        // EN: Calculate visible time window
        ...
        // 获取或复用屏幕窗口弹幕列表
        // EN: Get or reuse screen window list
        ...
        // 先绘制同步模式下的运行弹幕
        // EN: Draw sync mode running danmaku first
     ...
     * 处理具体的配置变更逻辑
     */
    // EN: Handle specific config change logic
    protected open fun handleOnDanmakuConfigChanged(
        ...
        when (tag) {
            ...
            DanmakuContext.DanmakuConfigTag.ALIGN_BOTTOM -> {
                ...
                // 其他配置变更不做特殊处理
                // EN: Other changes: no special handling
            }
        }
        return handled
    }
