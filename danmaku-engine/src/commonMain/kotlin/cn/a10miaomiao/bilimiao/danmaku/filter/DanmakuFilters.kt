...
class DanmakuFilters {
     ...
     * 弹幕过滤器接口
     */
    // EN: Danmaku filter interface
    interface IDanmakuFilter<T> {
        /**
         * 是否过滤该弹幕
         */
        // EN: Whether to filter this danmaku
     ...
     * 根据绘制耗时过滤弹幕
     */
    // EN: Filter by drawing elapsed time
     ...
     * 根据文本颜色白名单过滤
     */
    // EN: Filter by text color whitelist
     ...
     * 根据用户标识黑名单过滤
     */
    // EN: Filter by user ID blacklist
     ...
     * 根据用户hash黑名单过滤
     */
    // EN: Filter by user hash blacklist
     ...
     */
    // EN: Run all primary filters; first match sets mFilterParam bitmask
    fun filter(danmaku: BaseDanmaku, index: Int, totalsizeInScreen: Int,
