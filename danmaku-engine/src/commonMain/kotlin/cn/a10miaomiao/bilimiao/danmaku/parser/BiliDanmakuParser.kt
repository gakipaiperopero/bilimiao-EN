 ...
 */
 // EN: Bilibili XML danmaku parser. Parses Bilibili XML format: scrolling, fixed, and special danmaku (with animation).. Special danmaku text is JSON arrays with translation, alpha, rotation, path params.. XML format example:. Text. p attr: time(s),type,size,color,timestamp,poolId,userHash,danmakuId
class BiliDanmakuParser : BaseDanmakuParser() {
    ...
    override fun parse(): IDanmakus? {
            ...
            parseBiliDanmakuXml(
                ...
                onElementStart = { _, localName, _, attributes ->
                    ...
                    if (tagName == "d") {
                        ...
                        // EN: Text
                        // 0:时间(弹幕出现时间, 秒)
                        // EN: 0: Time (danmaku appear time, seconds)
                        // 1:类型(1从右至左|6从左至右|5顶端固定|4底端固定|7特殊弹幕)
                        // EN: 1: Type (1 R2L|6 L2R|5 top|4 bottom|7 special)
                        // 2:字号
                        // EN: 2: Font size
                        // 3:颜色
                        // EN: 3: Color
                ...
                onElementEnd = { _, localName, _ ->
                    ...
                    if (item != null) {
                        ...
                        if (tagName == "d") {
                            // 处理累积的文本内容
                            // EN: Handle accumulated text content
                            ...
                            if (rawText.isNotEmpty()) {
                                ...
                                // 解析特殊弹幕的 JSON 参数
                                // EN: Parse special danmaku JSON params
                            ...
                            // EN: Add valid danmaku (has text and duration) to result set
                ...
                onCharacters = { text ->
                    ...
                    // EN: Accumulate text; XML parser may call back multiple times
                    if (currentItem != null) {
        ...
        // 更新工厂视口状态
        // EN: Update factory viewport state
     ...
     */
     // EN: Parse special danmaku JSON array params. beginX/beginY: start coords (0.0~1.0 = percentage, >1 = pixels). alphaRange: alpha range, e.g. "0-1" = transparent to opaque. duration: animation duration (seconds). text: display text. rotateZ/rotateY: rotation angle. endX/endY: end coordinates. translationDuration: translation duration (ms). delay: translation start delay (ms). noStroke: "true" means no stroke. easing: "0"=Quadratic.easeOut, other=Linear.easeIn. pathData: SVG path, e.g. "M0,0L100,100L200,0"
    private fun parseSpecialDanmaku(danmaku: BaseDanmaku, text: String) {
        ...
        // EN: Parse alpha range: "0.5-1.0" or "1.0"
        ...
        // EN: Convert percentage coords to Bili player pixel coords
        ...
        // 是否有描边（去除阴影）
        // EN: Whether has stroke (remove shadow)
        if (textArr.size >= 12) {
    ...
    companion object {
         ...
         */
         // EN: Check if color is dark (HSV value < 0.1). Determines shadow/stroke color:. Dark text uses white shadow, light text uses black shadow.
         ...
         * 解码 XML 实体字符
         */
        // EN: Decode XML entity characters
         ...
         * 需要乘以播放器宽高转换为实际像素坐标。
         */
         // EN: Check if percentage number. In Bili special, numbers with decimals are percentages (0.0~1.0),. multiply by player width/height for pixel coords.
         ...
         */
         // EN: Simple JSON array parser. Parses JSON arrays like [value1,value2,"string",value4].. No kotlinx.serialization, manual quote/separator handling.. Supported types:. Numbers: 0.0, 6, -1.5. Quoted strings: "text content". Empty string: ""
            ...
            while (i < inner.length) {
                ...
                when {
                    ...
                    c == '"' && inString -> {
                        ...
                        // EN: Check for escaped quotes (\" in JSON = literal quote)
